...
class Controller(object):
    ...
    def __init__(self, simpy_env, conf):
        ...
        self.channels = [Channel(self, self.env, conf, i)
...
class Controller3(Controller):
    ...
    def __init__(self, simpy_env, conf, recorderobj):
        ...
        self.channels = [Channel3(self, self.env, conf, self.recorder, i)
                for i in range( self.n_channels_per_dev)]
...
class Channel(object):
    ...
    """
    def __init__(self, controller, simpy_env, conf, channel_id = None):
    ...
    def write_page(self, addr = None , data = None):
        ...
        with self.resource.request() as request:
            yield request
            yield self.env.timeout( int(self.program_time * (random.random()*0.2+0.9)) )

    def read_page(self, addr = None):
        with self.resource.request() as request:
            yield request
            yield self.env.timeout( int(self.read_time * (random.random()*0.2+0.9)))
...
class Channel2(Channel):
    ...
    It has recorder
    """
    def __init__(self, controller, simpy_env, conf, recorderobj, channel_id = None):
        super(Channel2, self).__init__(controller, simpy_env, conf, channel_id)
        self.recorder = recorderobj
...
class Channel3(Channel2):
    ...
    def write_page(self, tag, addr = None , data = None):
        ...
        with self.resource.request() as request:
            ...
            yield self.env.timeout( self.program_time )
            e = self.env.now
            # if addr == self.controller.physical_to_machine_page(33):
            #     log_msg("controller", 33, s, e)
            # if addr == self.controller.physical_to_machine_page(38):
            #     log_msg("controller", 33, s, e)

            self.recorder.count_me("channel", "write")
    ...
    def read_page(self, tag, addr = None):
        with self.resource.request() as request:
            ...
            # if addr == self.controller.physical_to_machine_page(33):
            #     log_msg("controller", 33, s, e)
            # if addr == self.controller.physical_to_machine_page(852043):
            #     log_msg("controller", 852043, s, e)
            # if addr == self.controller.physical_to_machine_page(184568676):
            #     log_msg("controller", 184568676, s, e)
            self.recorder.add_to_timer(
                self.counter_set_name(),
